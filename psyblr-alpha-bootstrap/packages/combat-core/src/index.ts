...
function applyStatus(state:CombatState,target:CombatUnitState,kind:StatusKind,sourceId:string,durationMs:number,value:number|undefined,events:CombatEvent[]) {
  const source=find(state, sourceId); const durationTicks=ticksFor(Math.round(durationMs * (1 + (source?.statusDurationPct ?? 0))));
...
function castSkill(state:CombatState,actor:CombatUnitState,events:CombatEvent[]) {
  ...
  if (skill.kind==='line_damage' && target) {
    ...
    for (const enemy of enemies) { const ex=enemy.x-actor.x,ez=enemy.z-actor.z,projection=ex*dx+ez*dz,cross=ex*dz-ez*dx; if (enemy.id===target.id || (projection>=0 && projection<=length && cross*cross<=width*width*length)) damage(state,actor,enemy,actor.atk*multiplier*(1+actor.skillPowerPct),'skill',events); }
  } else if (skill.kind==='aoe_slow' && target) {
    for (const enemy of enemies.filter(entry=>inRange(target,entry,radius))) { damage(state,actor,enemy,actor.atk*multiplier*(1+actor.skillPowerPct),'skill',events); applyStatus(state,enemy,'slow',actor.id,skill.durationMs ?? 1,skill.slowPercent,events); }
    ...
    for (const enemy of enemies.filter(entry=>inRange(actor,entry,radius))) damage(state,actor,enemy,actor.atk*multiplier*(1+actor.skillPowerPct),'skill',events);
  } else if (skill.kind==='taunt_shield') {
    const gained=Math.round(actor.atk*(skill.shieldMultiplier ?? 0)*(1+actor.skillPowerPct)); actor.shield+=gained; events.push(makeEvent(state.tick,'shield_changed',actor.id,actor.id,{shield:actor.shield,gained,vfx:'shield'}));
  ...
  } else if (skill.kind==='mark_vulnerability' && target) applyStatus(state,target,'vulnerability',actor.id,skill.durationMs ?? 1,Math.round((skill.vulnerabilityPercent ?? 0)*(1+actor.skillPowerPct)),events);
...
export function createCombatState(snapshot:CombatSnapshot,seed:number):CombatState {
  ...
  const state:CombatState={snapshot:structuredClone(snapshot),seed,tick:0,events,ended:false,winner:null,acceptedCommands:[],units:snapshot.units.slice().sort((a,b)=>a.id.localeCompare(b.id)).map(unit=>{
    const entry:CombatUnitState={...unit,basicAttackDamagePct:unit.basicAttackDamagePct ?? 0,skillPowerPct:unit.skillPowerPct ?? 0,statusDurationPct:unit.statusDurationPct ?? 0,maxHp:unit.hp,x:unit.spawnCell.x*1000,z:unit.spawnCell.z*1000,nextAttackTick:0,nextSkillReadyTick:unit.skill1?ticksFor(unit.skill1.initialDelayMs):Number.MAX_SAFE_INTEGER,skillReadyAnnounced:false,targetId:null,shield:0,statuses:[],dead:false};
...
export function stepCombat(state:CombatState,commands:readonly CombatCommand[]=[]):CombatStepResult {
  ...
  for (const unit of sortedAlive(state)) { if (stunned(unit)) continue; if (unit.skill1 && state.tick>=unit.nextSkillReadyTick && !unit.skillReadyAnnounced) { unit.skillReadyAnnounced=true; events.push(makeEvent(state.tick,'skill_ready',unit.id,null,{skillId:unit.skill1Id,vfx:'skill_ready'})); }
    ...
    else if (state.tick>=unit.nextAttackTick) { unit.nextAttackTick=state.tick+Math.max(1,Math.round(1000/(unit.attacksPerSecond*SIM_TICK_MS))); events.push(makeEvent(state.tick,'basic_attack',unit.id,target.id,{vfx:'basic_hit'})); damage(state,unit,target,unit.atk*(1+unit.basicAttackDamagePct),'basic',events); }
  }
