...
describe('tutorial checkpoint migration', () => {
  it('migrates valid PR04 checkpoints without discarding campaign state', () => {
    expect(migrateTutorialCheckpoint(v1)).toEqual({ ...v1, schemaVersion: 7, campPlacements: [], spawn: createTutorialSpawnRuntime(), merge: { appliedActionIds: [] }, raidDraft: createEmptyRaidSquadDraft(), raidSnapshot: null, raidResult: null });
  });
  it('round trips a v2 base checkpoint and rejects corrupt state', () => {
    ...
    expect(migrateTutorialCheckpoint(v2)).toEqual({ ...v2, schemaVersion: 7, spawn: createTutorialSpawnRuntime(), merge: { appliedActionIds: [] }, raidDraft: createEmptyRaidSquadDraft(), raidSnapshot: null, raidResult: null });
  ...
  it('migrates PR06 checkpoints while preserving the complete Spawn runtime', () => {
    ...
    expect(migrateTutorialCheckpoint(v3)).toEqual({ ...v3, schemaVersion: 7, merge: { appliedActionIds: [] }, raidDraft: createEmptyRaidSquadDraft(), raidSnapshot: null, raidResult: null });
  });
  it('migrates legacy raid state into the fresh 2/4/6 raid setup', () => {
    ...
    expect(migrateTutorialCheckpoint(v4)).toEqual({ ...v4, schemaVersion: 7, raidDraft: createEmptyRaidSquadDraft(), raidSnapshot: null, raidResult: null });
    ...
    expect(migrateTutorialCheckpoint(v5)).toEqual({ ...v5, schemaVersion: 7, raidDraft: createEmptyRaidSquadDraft(), raidSnapshot: null, raidResult: null });
  });
});
