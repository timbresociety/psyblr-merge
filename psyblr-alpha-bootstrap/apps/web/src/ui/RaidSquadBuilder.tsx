...
export function RaidSquadBuilder() {
  ...
  return <section className="raid-builder" data-tutorial-target="raid-squad-builder" data-testid="raid-squad-builder" aria-label="Raid squad builder"><header><div><span>RAID FIELD · ROUND {current.number}</span><strong>PLACE {current.slotCount} SUMMONS ON THE BLUE FIELD</strong></div><small>Drag a card to a blue cell, or tap a card then tap a blue cell. Origin and Function bonuses use this field only.</small></header><div className="raid-round-tabs" aria-label="Raid progress">{RAID_ROUND_DEFINITIONS.map((entry, index) => <div key={entry.id} data-active={entry.id === current.id} data-complete={index < results.length}><span>ROUND {entry.number} · {entry.slotCount}v{entry.slotCount}</span><b>{index < results.length ? results[index]!.outcome.toUpperCase() : entry.id === current.id ? `${field.length} / ${entry.slotCount} ON FIELD` : 'LOCKED'}</b></div>)}</div><p className="raid-field-help">DRAG OR TAP A SUMMON TO PLACE · {field.length}/{current.slotCount} READY</p><SummonPicker mode="raid" inventory={inventory} selectedInstanceIds={selectedId ? [selectedId] : []} deployedInstanceIds={field.map((placement) => placement.summonInstanceId)} onSelect={(instanceId) => select(current.id, instanceId)} onDragStart={(instanceId) => select(current.id, instanceId)} /><footer>{error && <p role="status">{error}</p>}<button type="button" className="raid-start" data-testid="raid-start" disabled={field.length !== current.slotCount} onClick={start}>START ROUND {current.number}</button>{status === 'error' && <button type="button" className="raid-start" onClick={retry}>RETRY</button>}</footer></section>;
}
