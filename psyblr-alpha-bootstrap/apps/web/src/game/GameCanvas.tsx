...
function WorldPointerBridge() {
  ...
  useEffect(() => {
    ...
    const resolve = (event: PointerEvent) => document.elementFromPoint(event.clientX, event.clientY) === canvas ? pointOnGround(camera, canvas, event.clientX, event.clientY) : null;
...
function CameraController() {
  ...
  const publish = useCallback(() => { const camera = app.root.findComponent('camera') as CameraComponent | undefined; const canvas = app.graphicsDevice.canvas as HTMLCanvasElement; if (camera && canvas && useGameStore.getState().scene === 'base') publishBaseWorldTargets(camera, canvas); }, [app]);
