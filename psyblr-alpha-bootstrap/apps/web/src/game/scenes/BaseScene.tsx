...
function CampCellEntity({ cell, material, protectedMaterial, moveMaterial, mergeMaterial, hoverMaterial, candidate, valid, mergeCandidate }: { cell: CampCell; material: StandardMaterial; protectedMaterial: StandardMaterial; moveMaterial: StandardMaterial; mergeMaterial: StandardMaterial; hoverMaterial: StandardMaterial; candidate: boolean; valid: boolean; mergeCandidate: boolean }) {
  ...
  return <Entity name={`camp-${cell.x}-${cell.y}`} position={[x, illuminated ? .12 : 0, z]} scale={[1.08, illuminated ? .15 : .07, 1.08]}>
    <Render type="box" material={mergeCandidate ? mergeMaterial : candidate ? hoverMaterial : illuminated ? protectedMaterial : valid ? moveMaterial : material} />
...
export function BaseScene() {
  ...
  const moveMaterial = useMaterial({ diffuse: '#1e3a5f', emissive: '#3b82f6', emissiveIntensity: .34, gloss: .5 });
  ...
  const hoverMaterial = useMaterial({ diffuse: '#a16207', emissive: '#fbbf24', emissiveIntensity: .95, gloss: .85 });
